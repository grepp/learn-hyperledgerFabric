...
app.post("/transaction/product", (req, res) => {
  ...
  fabric_client.getUserContext(userID, true).then((user_from_store) => {
    ...
    if (isProposalGood) {
      ...
      let peerserverCert = fs.readFileSync(path.join(__dirname, '../../crypto-config/peerOrganizations/store1.omarket.com/peers/peer0.store1.omarket.com/tls/ca.crt'));
      ...
      event_hub.setPeerAddr(ca_config.event_url, {
        ...
        'ssl-target-name-override': "peer0.store1.omarket.com",
      });
...
app.listen(3001, () => {
  ...
  let ordererserverCert = fs.readFileSync(path.join(__dirname, '../../crypto-config/ordererOrganizations/omarket.com/orderers/orderer1.omarket.com/tls/ca.crt'));
  let peerserverCert = fs.readFileSync(path.join(__dirname, '../../crypto-config/peerOrganizations/store1.omarket.com/peers/peer0.store1.omarket.com/tls/ca.crt'));
  ...
  order = fabric_client.newOrderer(ca_config.orderer_url, {
    ...
    'ssl-target-name-override': "orderer1.omarket.com",
  });
